component {
	...
	function configure() {
		...
		coldbox = {
			// Application Setup
			appName                  : getSystemSetting( "APPNAME", "CBFS-Demo" ),
			eventName                : "event",
		...
		moduleSettings = {
			"cbfs": {
				...
				"defaultDisk" : getSystemSetting("DEFAULT_STORAGE", "localStorage"),
				// Register the disks on the system
				"disks"       : {
					// Local file system
					"localStorage" : {
						"provider"   : "Local",
						"properties" : { "path" : "#controller.getAppRootPath()#.fileStorage" },
						"uploadMimeAccept": "*"
					},
					// S3
					"S3Storage" : {
						...
						"properties": {
							...
							"awsDomain": getSystemSetting("S3_DOMAIN", ""),
							"awsRegion": getSystemSetting("S3_REGION", ""),
							"defaultBucketName": getSystemSetting("S3_BUCKET", "")
						},
						"uploadMimeAccept": "*"
					}
				}
